...
export default async function handler(req, res) {
  ...
  try {
    const { emails, message, timestamp } = req.body;
        ...
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px; color: #333;">
          <div style="background: white; border-radius: 10px; padding: 30px; box-shadow: 0 10px 30px rgba(0,0,0,0.2);">
            ...
            <div style="text-align: center; margin-top: 30px;">
              <a href="${process.env.VITE_SITE_URL}/diary" style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: 600; transition: transform 0.2s;">
                📖 Abrir Diário
              </a>
            </div>
